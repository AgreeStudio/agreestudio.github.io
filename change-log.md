...
## Agree Studio 1.5.3 更新日志 (2020-01-16)
1. 新增分屏编辑功能，功能入口：文件右键 '在右侧打开'
2. 新增快捷打开最近打开的编辑器功能
3. vue-md预览视图新增快捷使用默认浏览器打开功能
4. bpmn编辑器样式修复、文本结构微调
5. markdown编辑器交互优化
6. 增加vue与js的代码注释块快捷提示（/** */）
7. 修复终端通过右侧点击切换到终端不能使用的问题
8. 完成vue的template部分ctrl+/ 注释问题
9. 修复vue 因template的属性问题造成 的输入辅助bug
10. 右击菜单打开终端
11. 修复导航复制路径缺陷，针对project,bank,platfrom文件夹特殊处理
12. 修复部分热键不响应问题
13. 设置首选项字体，主题运行时同步响应
14. 导航树删除文件/文件夹，解析静态html功能同步删除

## Agree Studio 1.5.0 更新日志 (2019-12-25)
1. 流程编辑器支持存储线位置
2. 前端工程不依赖des模式启动工程，预览视图仅支持简易页面预览
3. 优化内存占用问题

## Agree Studio 1.4.2 更新日志 (2019-11-29)
1. 流程编辑器
  - 添加泳道标注功能
  - 修复点击删除泳道时白屏bug
2. RCD编辑器循环节点的显示bug已修复
3. 新增错误视图
  - 展示服务工程错误并跳转展示节点/打开编辑器
  - 联动工程、导航选择
4. 导航支持多选删除文件、多选搜索
5. 文件搜索支持输入路径
6. vue编辑器 
  - 新增选择打开本地的HTML文件并预览的功能，且支持弹窗展示
  - 预览功能的合并及交互的变更，工具栏布局更改及样式优化，弹窗的样式优化及状态储存
  - 格式化 一行不超过限制数  属性不换行
  - template的标签内 不提示aui等组件提示

## Agree Studio 1.4.0 更新日志 (2019-11-20)
1. 流程编辑器
  - 添加泳道相关功能以及添加泳道属性区
  - 添加存在泳道时的自动布局功能
  - 支持F1查看技术组件源码
...
2. RCD编辑器在子结点迭代和类别上添加外部字段值（Expr）和内部数据字典（data）

3. 给自助工程modules中的vue文件增加metadata对象
...
5. 搜索
  - 内容搜索升级：去除高级模式、支持区分大小写、全字匹配功能
  - 文件搜索升级：独立弹窗与UI优化、搜索结果按匹配程度排序

6. csd编辑器新增加引入xm,ade文件修改显示名操作域

7. vue编辑器
  - 新增弹窗预览功能，且新弹窗支持原有功能
  - 新增选择打开的工程下的vue文件并预览的功能
  - 支持vue的格式化
  - 支持vue的 template与js部分一起语法检测

8. 新增绑定视图的首选项设置

9. 对于前端工程的models 下右击菜单 实现 编译部署的功能

## Agree Studio 1.3.5 更新日志 (2019-10-25)

- 热键支持自定义编辑
- 支持服务导出
- ade文件添加注释

## Agree Studio 1.3.3 更新日志 (2019-09-06)

- 修改新建数据字典文件校验规则；规定数据字典文件夹下数据字典文件唯一性；
- 交易文件支持提取管控因素
- 增加Module文件夹上右键弹窗勾选动态生成交易
- RCD编辑器导航树添加滚动条
- 组件资源展示视图支持存储为配置以便复用

## Agree Studio 1.3.2 更新日志 (2019-08-30)
1. 自定义组件编辑器
  - 修复无法匹配属性问题
  - 添加无属性、事件定义的健壮性提示
  - 导航支持声明文件的重命名
...
2. 组件资源展示视图
  - 初步实现自定义配置组件库属性提示功能（测试版功能，谨慎使用）
  - 支持恢复默认配置
  - 暂不支持配置持久化存储

3. 其它
  - 全局搜索热键改为 ctrl + p ，与vscode一致
